ARG BRANCH=master
FROM metwork/mfcom-centos6-buildimage:${BRANCH} as yum_cache
ARG BRANCH
...
ARG CACHEBUST=0
RUN yum --disablerepo=* --enablerepo=metwork_${BRANCH} -q list metwork-mcom* 2>/dev/null |sort |md5sum |awk '{print $1;}' > /tmp/yum_cache
